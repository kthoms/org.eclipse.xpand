...
package org.eclipse.xpand3.internal.frontend.node2ast;
...
import org.eclipse.xpand3.internal.frontend.parser.AbstractXpand3NodeSwitch;
...
import org.eclipse.xpand3.parser.TransformationException;
...
public class Node2AstTransformer extends
	...
	private void createAndAddImplicitlyDeclaredParameter(String parameterName,
			CompositeNode parameterTypeNode, EList<DeclaredParameter> parameters) {
		if (parameterTypeNode != null) {
			DeclaredParameter dp = xpandFactory.createDeclaredParameter();
			dp.setName(createIdentifier(parameterName, parameterTypeNode));
			dp.setType((Identifier) caseSimpleTypeNode(parameterTypeNode));
			setPosition(dp, parameterTypeNode);
			parameters.add(dp);
		}
	}
	...
	public SyntaxElement caseCheckNode(CheckNode node)
		...
		createAndAddImplicitlyDeclaredParameter("this", node.getType(), check
				.getParams());
		return check;
	}
	...
	public SyntaxElement caseDefinitionAroundNode(DefinitionAroundNode node)
		...
		createAndAddImplicitlyDeclaredParameter("this", node.getType(),
				parameters);
	...
	public SyntaxElement caseExtensionNode(ExtensionNode node) {
		...
		if (node.getCreate() != null) {
			extension = declarationFactory.createCreateExtension();
			String createdName = (node.getCreatedName() != null) ? node
					.getCreatedName().text() : "this";
			createAndAddImplicitlyDeclaredParameter(createdName, node
					.getReturnType(), extension.getParams());
			((CreateExtension) extension).setToBeCreated(extension.getParams()
					.get(0));
			extension.setCached(true);
		} else {
			if (node.getCreatedName() != null) {
				throw new TransformationException(
						"Only create extension can declare identifier here",
						node.getCreatedName());
			}
			extension = declarationFactory.createExtension();
			extension.setCached(node.getCached() != null);
		}
		setPosition(extension, node);
		extension.setName((Identifier) caseIdentifierNode(node.getName()));
		if (node.getReturnType() != null) {
			extension
					.setReturnType((Identifier) doSwitch(node.getReturnType()));
		}
		DeclaredParameterListNode paramListNode = (DeclaredParameterListNode) node
				.getParamList();
