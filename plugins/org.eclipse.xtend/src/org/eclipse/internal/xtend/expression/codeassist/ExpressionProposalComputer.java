...
public class ExpressionProposalComputer implements ProposalComputer {
     ...
     * @return
     */
    public List<Object> computeProposals(final String txt, ExecutionContext ctx, final ProposalFactory factory) {
        final String[] s = computePrefixAndTargetExpression(txt);
        final String prefix = s[0];
        final String expressionString = s[1];
        ctx = computeExecutionContext(txt, ctx);
        ...
        if (expressionString != null) {
            ...
            implicitVariableType = new ExpressionFacade(ctx).analyze(expressionString, issues);
            if (implicitVariableType == null)
                return Collections.emptyList();
        }
        if (implicitVariableType == null) {
            // variables
            final Map<String, Variable> vars = ctx.getVisibleVariables();
            for (final Iterator<String> iter = vars.keySet().iterator(); iter.hasNext();) {
                ...
                if (varName.toLowerCase().startsWith(prefix.toLowerCase())) {
                    final Object o = ctx.getVariable(varName).getValue();
            ...
            final Set<? extends Extension> exts = ctx.getAllExtensions();
    ...
    private final static Pattern COL_OP = Pattern
            .compile("((select|collect|exists|notExists|reject|forEach|sortBy)\\s*\\(\\s*(\\w+)\\s*\\|)|(\\()|(\\))");

    private final static Pattern LET = Pattern.compile(".*let\\s*(\\w+)\\s*=\\s*([^:]+):([^:]*)");

    public final static ExecutionContext computeExecutionContext(final String txt, ExecutionContext ctx) {
