...
public abstract class QuickMenuAction extends Action {
	...
	public String addShortcut(String menuText) {
		...
		Object[] args = new Object[] { menuText, shortcut };
	...
	protected Point computeMenuLocation(Tree tree) {
		...
		case 1:
			...
			if (intersect != null && intersect.height == bounds.height) {
				return new Point(Math.max(0, bounds.x + getAvarageCharWith(tree) * QuickMenuAction.CHAR_INDENT),
		...
		default:
			...
			Point result = findBestLocation(getIncludedPositions(rectangles, clientArea), tree
					.toControl(cursorLocation));
			if (result != null) {
				result.x = result.x + getAvarageCharWith(tree) * QuickMenuAction.CHAR_INDENT;
			}
			return result;
		}
	}
	...
	protected Point computeMenuLocation(Table table) {
		...
		case 1: {
			...
			Rectangle intersect = clientArea.intersection(bounds);
			if (intersect != null && intersect.height == bounds.height) {
				return new Point(Math.max(0, bounds.x + iBounds.width + getAvarageCharWith(table)
						* QuickMenuAction.CHAR_INDENT), bounds.y + bounds.height);
			} else {
				return null;
			}
		}
		default: {
			...
			Point result = findBestLocation(getIncludedPositions(rectangles, clientArea), table
					.toControl(cursorLocation));
			if (result != null) {
				result.x = result.x + iBounds.width + getAvarageCharWith(table) * QuickMenuAction.CHAR_INDENT;
			}
			return result;
		}
		}
